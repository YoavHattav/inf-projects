...
status_t TrieInsert(trie_t *trie, char *str)
{
	...
	return (InsertHelper(trie->root, runner));
}
...
static void FreeLeafHelper(trie_node_t *root, char *str)
{
	if (NULL == root)
	{
		return;
	...
	FreeLeafHelper(root->direction[(int)*str], str + 1);
}
...
void TrieFreeLeaf(trie_t *trie, char *str)
{
	...
	FreeLeafHelper(trie->root, runner);
}
